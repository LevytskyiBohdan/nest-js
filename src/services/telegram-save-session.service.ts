...
export class TelegramSaveSessionService {
  ...
  async number(ctx: Context) {
    const number = (ctx.update as any)?.message?.text?.replace(
      /\/number(| )/,
      '',
    );
    ...
    if (phoneNumber.isValid()) {
      await ctx.reply(
        await this.saveSession(telegramUserID, phoneNumber, telegramUserID),
      );
    }
  }
    ...
    try {
      ...
      client
        ...
        .then(async (res) => {
          ...
          this.logger.log(
            `Adding number ${phoneNumber.getNumber()} to our db with session: ${telegramSession}`,
          );
